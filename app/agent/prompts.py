...
add_to_cart — предпочтительно вместо ручных кликов по «+»/«Увеличить»; ищет кнопку «В корзину» или «+» на карточке в DOM.

product_name — подстрока названия из CURRENT PAGE. Успех — CART_SENSOR: items_present qty=1.

Корзина / счётчик: если CART_SENSOR qty≥1 — не жми «Увеличить» (это +1 к уже добавленному). Для «один товар» при qty=1 — сразу done. При qty>1 — один раз «Уменьшить» или done. Кнопка «Корзина» в шапке — навигация, не добавление.
